...
#define LFS_PORT_FLASH_ERROR_FLAGS                                             \
...
static bool lfs_port_range_valid(lfs_block_t block, lfs_off_t off,
                                 lfs_size_t size) {
...
lfs_port_flash_program(uint32_t address, const uint8_t *data, size_t size) {
...
int lfs_port_format(lfs_t *lfs) { return lfs_format(lfs, &lfs_port_cfg); }

int lfs_port_mount(lfs_t *lfs) { return lfs_mount(lfs, &lfs_port_cfg); }

int lfs_port_unmount(lfs_t *lfs) { return lfs_unmount(lfs); }