 ...
 * @LastEditTime: 2026-02-07 16:52:22
 * @FilePath: /11_rocketpi_uart_shell_microrl/src/main.c
 * @Description:集成 microrl 命令行库，在 USART2（115200 8N1）上提供交互式
 * shell，并扩展了简单的板载功能控制。 不用客气，这是你应该谢的! Copyright (c)
 * 2026 by ${git_name_email}, All Rights Reserved.
 */
...
static void shell_print(const char *str) {
  ...
  while (*str != '\0') {
    if (*str == '\n') {
      ...
      HAL_UART_Transmit(&huart2, (uint8_t *)newline, sizeof(newline),
                        HAL_MAX_DELAY);
    } else {
...
static void shell_queue_char(uint8_t value) {
  uint16_t next_head =
      (uint16_t)((s_shell_rx_head + 1U) % SHELL_RX_BUFFER_SIZE);
  ...
  while (s_shell_rx_head != s_shell_rx_tail) {
    ...
    if (ch == '\r') {
      ...
      if (s_shell_rx_head != s_shell_rx_tail) {
        ...
        if (peek == '\n') {
          s_shell_rx_tail =
              (uint16_t)((s_shell_rx_tail + 1U) % SHELL_RX_BUFFER_SIZE);
        }
      }
    }
