...
static const uint8_t g_dummy_payload[] =
    ...
    "--------------------------------\r\n"
    "This file is generated inside firmware and streamed via the YMODEM "
    "sender.\r\n"
    "You can verify the protocol end-to-end by checking that this sentence and "
    "the\r\n"
    ...
    "\r\n"
    "Have fun testing!\r\n";
static const uint32_t g_dummy_payload_len =
    (uint32_t)(sizeof(g_dummy_payload) - 1U);
...
static int dummy_seek_any(void *fh, int64_t off, int whence) {
  ...
  case 0:
    ...
    break;
  case 1:
    target = (int64_t)h->pos + off;
    break;
  case 2:
    target = (int64_t)h->len + off;
    break;
  default:
    return -1;
  }
  if (target < 0 || target > (int64_t)h->len)
    return -1;
...
static uint32_t flash_store_free_bytes(void) {
  if (g_flash_store.cursor >= g_flash_store.end)
    return 0;
...
static void *flash_open_write(const char *out_dir, const char *name) {
  (void)out_dir;
  (void)name;
  if (g_flash_store.busy)
    return NULL;
  if (!g_flash_store.erased) {
    if (flash_store_erase_all() != 0)
      return NULL;
...
static int flash_write_bytes(void *fh, const void *buf, int len) {
  ...
  for (int i = 0; i < len; ++i) {
    ...
    HAL_StatusTypeDef st =
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, addr, (uint64_t)src[i]);
    if (st != HAL_OK) {
      HAL_FLASH_Lock();
      return -1;
    }
...
static int stm32_putc(YPort *p, uint8_t ch) {
  ...
  if (!s || !s->huart)
    return -1;
  if (HAL_UART_Transmit(s->huart, &ch, 1, 1000) == HAL_OK)
    return 1;
  return -1;
}
static int stm32_getc(YPort *p, uint8_t *ch, int ms) {
  ...
  if (!s || !s->huart)
    return -1;
  if (HAL_UART_Receive(s->huart, ch, 1,
                       (ms >= 0) ? (uint32_t)ms : HAL_MAX_DELAY) == HAL_OK)
    return 1;
  return 0;
}
static int stm32_write(YPort *p, const void *buf, int len) {
  ...
  if (!s || !s->huart)
    return -1;
  if (HAL_UART_Transmit(s->huart, (uint8_t *)buf, (uint16_t)len, 5000) ==
      HAL_OK)
    return len;
  return -1;
}
...
static void cli_log(const char *s) { printf("%s\r\n", s); }
static void cli_prog(const char *tag, const char *name, uint64_t done,
                     int64_t total) {
  (void)tag;
  (void)name;
  (void)done;
