 ...
 * @LastEditTime: 2026-03-08 11:40:42
 * @FilePath: /41_rocketpi_mbedtls/src/main.c
 * @Description:此工程为rocketpi验证加密库可用性
 * 不用客气，这是你应该谢的!
...
static const char RSA_N_HEX[] = "9292758453063D803DD603D5E777D788"
...
static const char RSA_D_HEX[] = "24BF6185468786FDD303083D25E64EFC"
...
static const char RSA_P_HEX[] = "C36D0EB7FCD285223CFB5AABA5BDA3D8"
                                "2C01CAD19EA484A87EA4377637E75500"
                                "FCB2005C5C7DD6EC4AC023CDA285D796"
                                "C3D9E75E1EFC42488BB4F1D13AC30A57";
static const char RSA_Q_HEX[] = "C000DF51A7C77AE8D7C7370C1FF55B69"
...
static int run_aes_test(void) {
  ...
  const uint8_t expected_cipher[16] = {0x3a, 0xd7, 0x7b, 0xb4, 0x0d, 0x7a,
  ...
  if ((ret = mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_ENCRYPT, plain, cipher)) !=
      0) {
    goto cleanup;
  }
  ...
  if ((ret = mbedtls_aes_crypt_ecb(&aes, MBEDTLS_AES_DECRYPT, cipher,
                                   decrypted)) != 0) {
    goto cleanup;
  }
...
static int run_rsa_test(void) {
  ...
  do {
    ...
    if ((ret = mbedtls_rsa_pkcs1_encrypt(
             &rsa, pseudo_entropy_source, &seed, MBEDTLS_RSA_PUBLIC,
             sizeof(message) - 1U, message, ciphertext)) != 0) {
      printf("RSA encrypt failed: %d\r\n", ret);
      break;
    }

    if ((ret = mbedtls_rsa_pkcs1_decrypt(&rsa, pseudo_entropy_source, &seed,
                                         MBEDTLS_RSA_PRIVATE, &olen, ciphertext,
  ...
  if (ret == 0) {
    if ((olen != (sizeof(message) - 1U)) ||
        (memcmp(decrypted, message, olen) != 0)) {
      ret = -1;
      goto cleanup;
    }
...
static int run_sha256_test(void) {
  ...
  const uint8_t expected_hash[32] = {
      0xb9, 0x2f, 0x7c, 0xae, 0xa1, 0x4f, 0x3b, 0x71, 0xfe, 0xbb, 0x57,
      0x98, 0x75, 0x65, 0x17, 0x7d, 0xb1, 0x03, 0x11, 0x61, 0x93, 0xd7,
      0x9e, 0x85, 0x1e, 0x17, 0x6b, 0x13, 0x91, 0x88, 0xb3, 0x63};
  uint8_t hash[32];
  ...
  if ((ret = mbedtls_sha256_update_ret(&ctx, message, sizeof(message) - 1U)) !=
      0) {
    goto cleanup;
  }
