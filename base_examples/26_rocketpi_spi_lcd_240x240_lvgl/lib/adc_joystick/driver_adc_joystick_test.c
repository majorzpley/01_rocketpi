...
static void adc_joystick_test_map_direction(const adc_joystick_sample_t *sample,
                                            char *out, uint32_t *n);
...
static uint16_t adc_joystick_test_scale_permille(uint16_t raw) {
  uint32_t permille =
      ((uint32_t)raw * 1000U + (ADC_JOYSTICK_TEST_ADC_MAX_VALUE / 2U)) /
...
static uint8_t adc_joystick_test_read_key_internal(void) {
  GPIO_PinState state =
      HAL_GPIO_ReadPin(ADC_JOYSTICK_KEY_GPIO_Port, ADC_JOYSTICK_KEY_Pin);

  printf("ADC_JOYSTICK_KEY state:%d\n", state);
...
uint8_t adc_joystick_test_run(uint32_t sample_count, uint32_t delay_ms) {
  const uint32_t count = (sample_count == 0U)
                             ? ADC_JOYSTICK_TEST_DEFAULT_SAMPLE_COUNT
                             : sample_count;
  const uint32_t delay =
      (delay_ms == 0U) ? ADC_JOYSTICK_TEST_DEFAULT_DELAY_MS : delay_ms;
  ...
  for (uint32_t i = 0U; i < count; ++i) {
    ...
    printf("adc joystick: #%lu X=%4u (%3lu.%01lu%%) Y=%4u (%3lu.%01lu%%) "
           "KEY=%s\r\n",
           (unsigned long)(i + 1U), (unsigned int)sample.x.raw,
           (unsigned long)(sample.x.permille / 10U),
           (unsigned long)(sample.x.permille % 10U), (unsigned int)sample.y.raw,
 ...
 */
uint8_t adc_joystick_send_udlr_uart(uint32_t sample_count, uint32_t delay_ms,
  ...
  if (sample_count == 0U) {
    ...
    while (1) {
      ...
      if (n > 0U) {
        if (HAL_UART_Transmit(huart, (uint8_t *)out, (uint16_t)n, 10U) !=
            HAL_OK) {
          return 3U;
        }
      }
