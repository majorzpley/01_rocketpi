 ...
 * @LastEditTime: 2026-03-07 17:56:19
 * @FilePath: /40_rocketpi_crc/src/main.c
 * @Description:此工程为rocketpi CRC校验验证
 * 不用客气，这是你应该谢的!
...
static void CRC_LogResults(void) {
  ...
  for (size_t i = 0; i < g_crc_result_count; ++i) {
    ...
    len = snprintf(line, sizeof(line), "%s exp=0x%08lX act=0x%08lX %s\r\n",
                   res->name, (unsigned long)res->expected,
                   (unsigned long)res->actual, status);
