 ...
 * @Date: 2026-03-07 17:21:58
 * @LastEditors: majorzpley wyx1214844230@outlook.com
 * @LastEditTime: 2026-03-07 17:22:23
 * @FilePath: /40_rocketpi_crc/lib/crc/crc_test.c
 * @Description:
 * 不用客气，这是你应该谢的!
...
static const uint8_t kTestPayload[] = {'1', '2', '3', '4', '5',
                                       '6', '7', '8', '9'};
...
static uint32_t crc_reference_msb(const uint8_t *data, size_t len,
                                  uint8_t width, uint32_t polynomial,
                                  uint32_t seed, uint32_t xor_out) {
  if (width == 0U) {
    return 0U;
  }

  const uint64_t mask =
      (width >= 32U) ? 0xFFFFFFFFULL : ((1ULL << width) - 1ULL);
...
static uint32_t crc_reference_lsb(const uint8_t *data, size_t len,
                                  uint8_t width, uint32_t polynomial,
                                  uint32_t seed, uint32_t xor_out) {
  if (width == 0U) {
    return 0U;
...
static const crc_test_case_t kTestCases[] = {
    {"crc32_ieee", dut_crc32_ieee, NULL, 32U, 0xEDB88320U, 0xFFFFFFFFU,
     0xFFFFFFFFU, true},
    {"crc32_ieee_stream", dut_crc32_ieee_chunked, NULL, 32U, 0xEDB88320U,
     0xFFFFFFFFU, 0xFFFFFFFFU, true},
    {"crc32_c", dut_crc32_c, NULL, 32U, 0x82F63B78U, 0xFFFFFFFFU, 0xFFFFFFFFU,
     true},
    {"crc32_k_4_2", dut_crc32_k, NULL, 32U, 0x93A409EBU, 0x00000000U,
     0x00000000U, false},
    {"crc24_pgp", dut_crc24_pgp, NULL, 24U, CRC24_PGP_POLY,
     CRC24_PGP_INITIAL_VALUE, 0x000000U, false},
    {"crc24_pgp_stream", dut_crc24_pgp_chunked, NULL, 24U, CRC24_PGP_POLY,
     CRC24_PGP_INITIAL_VALUE, 0x000000U, false},
    {"crc24q_rtcm3", dut_crc24q, NULL, 24U, 0x01864CFBU, 0x000000U, 0x000000U,
     false},
    {"crc16", dut_crc16, NULL, 16U, 0x1021U, 0xFFFFU, 0x0000U, false},
    {"crc16_ccitt", dut_crc16_ccitt, NULL, 16U, 0x8408U, 0xFFFFU, 0x0000U,
     true},
    {"crc16_reflect", dut_crc16_reflect, NULL, 16U, 0xA001U, 0xFFFFU, 0x0000U,
     true},
    {"crc16_itu_t", dut_crc16_itu_t, NULL, 16U, 0x1021U, 0x0000U, 0x0000U,
     false},
    ...
    {"crc4_reversed", dut_crc4_lsb, ref_crc4_reversed, 4U, 0x03U, 0x00U, 0x00U,
     true},
...
static uint32_t run_reference(const crc_test_case_t *tc) {
  ...
  if (tc->lsb_variant) {
    return crc_reference_lsb(kTestPayload, kTestPayloadSize, tc->width,
                             tc->polynomial, tc->seed, tc->xor_out);
  }

  return crc_reference_msb(kTestPayload, kTestPayloadSize, tc->width,
                           tc->polynomial, tc->seed, tc->xor_out);
}
...
/* 返回当前注册的CRC测试用例数量 */
size_t crc_get_test_count(void) { return ARRAY_SIZE(kTestCases); }
