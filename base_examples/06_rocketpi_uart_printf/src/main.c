 ...
 * @LastEditTime: 2026-02-09 13:27:30
 * @FilePath: /06_rocketpi_uart_printf/src/main.c
 * @Description:串口打印实验(实现串口打印在不同编译器下<gcc或armcc>的适配)
 * 不用客气，这是你应该谢的!
...
int main(void) {
  ...
  while (1) {
    ...
    // bug为什么这里uart_hexdump会把之前的printf内容输出至串口，但是注释掉此句就无法输出printf的内容
    // !是由于pio默认将lib目录下的库编译成了静态库，导致其会用到HAL库的发送函数
