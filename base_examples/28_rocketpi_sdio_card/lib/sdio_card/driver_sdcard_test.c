...
static void sdcard_test_fill_pattern(uint8_t *buffer, uint32_t length,
                                     uint32_t seed);
static uint8_t sdcard_test_verify_pattern(const uint8_t *buffer,
...
static HAL_StatusTypeDef sdcard_test_write_blocks(uint8_t *buffer,
                                                  uint32_t block_addr,
                                                  uint32_t block_count);
static HAL_StatusTypeDef sdcard_test_read_blocks(uint8_t *buffer,
                                                 uint32_t block_addr,
                                                 uint32_t block_count);
...
#else
  return HAL_SD_WriteBlocks(&hsd, buffer, block_addr, block_count,
                            HAL_MAX_DELAY);
#endif
}
...
#else
  return HAL_SD_ReadBlocks(&hsd, buffer, block_addr, block_count,
                           HAL_MAX_DELAY);
#endif
}
...
static uint8_t sdcard_test_verify_pattern(const uint8_t *buffer,
  ...
  for (uint32_t i = 0U; i < length; ++i) {
    ...
    if (buffer[i] != expected) {
      printf("sdcard: data mismatch at byte %lu (expected=0x%02X, "
             "read=0x%02X)\r\n",
             (unsigned long)i, expected, buffer[i]);
...
static void sdcard_test_print_info(const HAL_SD_CardInfoTypeDef *info) {
  const uint64_t capacity_bytes =
      (uint64_t)info->LogBlockNbr * (uint64_t)info->LogBlockSize;
  ...
  printf(
      "sdcard: RCA=%lu, logical blocks=%lu, logical block size=%lu bytes\r\n",
      (unsigned long)info->RelCardAdd, (unsigned long)info->LogBlockNbr,
...
static uint8_t sdcard_test_process_range(const HAL_SD_CardInfoTypeDef *info,
  ...
  if (max_chunk_blocks == 0U) {
    printf("sdcard: configured buffer %lu bytes is smaller than a block (%lu "
           "bytes)\r\n",
           (unsigned long)SDCARD_TEST_BUFFER_BYTES, (unsigned long)block_size);
  ...
  if (block_addr >= info->LogBlockNbr) {
    printf("sdcard: start block %lu is outside the card (max %lu)\r\n",
           (unsigned long)block_addr, (unsigned long)(info->LogBlockNbr - 1U));
  ...
  printf("sdcard: verifying %lu blocks from address %lu (chunk=%lu blocks)\r\n",
         (unsigned long)total_blocks, (unsigned long)block_addr,
         (unsigned long)max_chunk_blocks);

  while (remaining_blocks > 0U) {
    const uint32_t chunk_blocks = (remaining_blocks < max_chunk_blocks)
                                      ? remaining_blocks
                                      : max_chunk_blocks;
    ...
    if (status != HAL_OK) {
      printf("sdcard: write failed at block %lu (err=0x%08lX)\r\n",
             (unsigned long)current_block, (unsigned long)hsd.ErrorCode);
    ...
    if ((processed_blocks - last_report) >= SDCARD_TEST_PROGRESS_STEP_BLOCKS ||
        remaining_blocks == 0U) {
      printf("sdcard: progress %lu/%lu blocks\r\n",
             (unsigned long)processed_blocks, (unsigned long)total_blocks);
...
/* 从指定起始块开始做一次读写校验 */
uint8_t sdcard_test_block_read_write(uint32_t block_addr,
                                     uint32_t block_count) {
  HAL_SD_CardInfoTypeDef info;
