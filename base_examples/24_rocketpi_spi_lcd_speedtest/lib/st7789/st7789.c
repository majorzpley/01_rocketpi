...
void ST7789_DrawPixel(uint16_t x, uint16_t y, uint16_t color) {
  if (x >= ST7789_X_RES || y >= ST7789_Y_RES)
    return;
...
void ST7789_FillRect(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye,
  ...
  if (xs >= ST7789_X_RES || ys >= ST7789_Y_RES)
    return;
  if (xe >= ST7789_X_RES)
    xe = ST7789_X_RES - 1;
  if (ye >= ST7789_Y_RES)
...
void ST7789_DrawBitLine16BPP(uint16_t xs, uint16_t y, const uint8_t *p,
                             uint16_t xsize) {
  if (y >= ST7789_Y_RES || xs >= ST7789_X_RES)
    return;
  if (xsize == 0)
    return;
  if (xs + xsize > ST7789_X_RES)
    xsize = ST7789_X_RES - xs;
...
void ST7789_ShowChar(uint16_t x, uint16_t y, uint8_t ch, FontDef font,
  ...
  /* 越界裁剪 */
  if (x + w - 1 >= ST7789_X_RES || y + h - 1 >= ST7789_Y_RES)
    return;
...
void ST7789_TestFrameRate(void) {
  ...
  uint16_t x = (text_width < ST7789_X_RES)
                   ? (uint16_t)((ST7789_X_RES - text_width) / 2U)
                   : 0;
  uint16_t y = (Font16x24.height < ST7789_Y_RES)
                   ? (uint16_t)((ST7789_Y_RES - Font16x24.height) / 2U)
                   : 0;
