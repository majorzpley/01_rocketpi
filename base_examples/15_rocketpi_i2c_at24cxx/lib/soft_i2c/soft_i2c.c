...
static soft_i2c_status_t soft_i2c_drive_scl(soft_i2c_bus_t *bus,
                                            soft_i2c_pin_state_t state);
static soft_i2c_status_t soft_i2c_drive_sda(soft_i2c_bus_t *bus,
                                            soft_i2c_pin_state_t state);
...
static soft_i2c_status_t soft_i2c_write_byte(soft_i2c_bus_t *bus,
                                             uint8_t value);
static soft_i2c_status_t soft_i2c_read_byte(soft_i2c_bus_t *bus, uint8_t *value,
                                            int last_byte);

/* 初始化软I2C总线：确认GPIO操作回调存在并设置默认延时 */
soft_i2c_status_t soft_i2c_bus_init(soft_i2c_bus_t *bus) {
  if ((bus == NULL) || (bus->scl.write == NULL) || (bus->scl.read == NULL) ||
...
/* 先写后读的组合事务，实现典型寄存器访问 */
soft_i2c_status_t soft_i2c_master_write_read(soft_i2c_bus_t *bus,
                                             uint8_t address,
...
static void soft_i2c_delay(const soft_i2c_bus_t *bus) {
  ...
  uint32_t ticks = (bus == NULL || bus->delay_ticks == 0U)
                       ? SOFT_I2C_DEFAULT_DELAY_TICKS
                       : bus->delay_ticks;
...
static soft_i2c_status_t soft_i2c_write_byte(soft_i2c_bus_t *bus,
  ...
  for (int8_t bit = 7; bit >= 0; --bit) {
    soft_i2c_pin_state_t state =
        ((value >> bit) & 0x01U) ? SOFT_I2C_PIN_SET : SOFT_I2C_PIN_RESET;
  ...
  return (ack == SOFT_I2C_PIN_RESET) ? SOFT_I2C_STATUS_OK
...
static soft_i2c_status_t soft_i2c_read_byte(soft_i2c_bus_t *bus, uint8_t *value,
  ...
  status = soft_i2c_drive_sda(bus, last_byte ? SOFT_I2C_PIN_SET
                                             : SOFT_I2C_PIN_RESET);
  if (status != SOFT_I2C_STATUS_OK) {
    return status;
  }
