...
typedef struct {
  ...
  uint8_t
      rx_buffer[MG58F18_RADAR_RX_BUFFER_SIZE]; /**< 接收缓冲，供空闲中断使用 */
} mg58f18_radar_hal_t;
...
uint32_t mg58f18_radar_interface_hw_get_tick(void) { return HAL_GetTick(); }
...
bool mg58f18_radar_interface_hw_read_io(void) {
  const GPIO_PinState state =
      HAL_GPIO_ReadPin(RADAR_IO_GPIO_Port, RADAR_IO_Pin);
