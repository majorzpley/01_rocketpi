...
#define MG58F18_RADAR_CHECKSUM_INDEX                                           \
  5U /**< Data1^Data2^Data3^Data4 的放置位置 */
 ...
 */
static void
mg58f18_radar_core_store_frame(mg58f18_radar_core_t *ctx,
 ...
 */
static void mg58f18_radar_core_begin_transaction(mg58f18_radar_core_t *ctx,
                                                 uint8_t command) {
 ...
 */
static void mg58f18_radar_core_abort(mg58f18_radar_core_t *ctx,
                                     mg58f18_radar_status_t reason) {
 ...
 */
static mg58f18_radar_status_t
mg58f18_radar_core_process_byte(mg58f18_radar_core_t *ctx, uint8_t byte) {
  ...
  const uint8_t checksum = mg58f18_radar_compute_checksum(
      ctx->parser.buffer[1], ctx->parser.buffer[2], ctx->parser.buffer[3],
      ctx->parser.buffer[4]);
  const bool checksum_ok =
      (checksum == ctx->parser.buffer[MG58F18_RADAR_CHECKSUM_INDEX]);
  const bool tail_ok =
      (ctx->parser.buffer[MG58F18_RADAR_TAIL_INDEX] == MG58F18_RADAR_TAIL_BYTE);

  if (checksum_ok && tail_ok) {
    mg58f18_radar_core_store_frame(ctx, ctx->parser.buffer);

    if (ctx->awaiting_reply &&
        ctx->last_frame.command == ctx->pending_command) {
      ctx->awaiting_reply = false;
      ctx->last_error = MG58F18_RADAR_STATUS_OK;
    } else {
      ctx->last_error = MG58F18_RADAR_STATUS_OK;
  ...
  const bool restart =
      (ctx->parser.buffer[MG58F18_RADAR_TAIL_INDEX] == MG58F18_RADAR_HEAD_BYTE);
...
static mg58f18_radar_status_t
mg58f18_radar_core_process_bytes(mg58f18_radar_core_t *ctx, const uint8_t *data,
                                 size_t length) {
...
static bool
mg58f18_radar_core_is_awaiting_reply(const mg58f18_radar_core_t *ctx) {
  return ctx->awaiting_reply;
}

static mg58f18_radar_status_t
mg58f18_radar_core_get_last_error(const mg58f18_radar_core_t *ctx) {
  return ctx->last_error;
}

static const mg58f18_radar_frame_t *
mg58f18_radar_core_last_frame(const mg58f18_radar_core_t *ctx) {
 ...
 */
static mg58f18_radar_status_t
mg58f18_radar_wait_for_reply(uint32_t timeout_ms) {
 ...
 */
static mg58f18_radar_status_t
mg58f18_radar_send_command_raw(uint8_t command, uint8_t data2, uint8_t data3,
                               uint8_t data4, mg58f18_radar_frame_t *response) {
  if (!s_initialised) {
  ...
  frame[3] = data3;
  frame[4] = data4;
  frame[MG58F18_RADAR_CHECKSUM_INDEX] =
      mg58f18_radar_compute_checksum(command, data2, data3, data4);
  frame[MG58F18_RADAR_TAIL_INDEX] = MG58F18_RADAR_TAIL_BYTE;

  mg58f18_radar_core_begin_transaction(&s_core, command);

  mg58f18_radar_status_t status = mg58f18_radar_interface_hw_send(
      frame, MG58F18_RADAR_FRAME_SIZE, MG58F18_RADAR_DEFAULT_TIMEOUT_MS);
 ...
 */
static mg58f18_radar_status_t
mg58f18_radar_send_command_u24(uint8_t command, uint32_t value,
...
uint32_t mg58f18_radar_frame_get_u24(const mg58f18_radar_frame_t *frame) {
  ...
  return ((uint32_t)frame->data2 << 16) | ((uint32_t)frame->data3 << 8) |
         frame->data4;
}
...
mg58f18_radar_set_distance_threshold(uint16_t threshold) {
  ...
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_DISTANCE_THRESHOLD,
                                   0x00U, data3, data4);
}
...
mg58f18_radar_get_distance_threshold(uint16_t *threshold) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_DISTANCE_THRESHOLD, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (threshold != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_delay_ms(uint32_t delay_ms) {
  ...
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_u24(
      MG58F18_RADAR_CMD_SET_DELAY_TIME, value, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
...
mg58f18_radar_status_t mg58f18_radar_get_delay_ms(uint32_t *delay_ms) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_DELAY_TIME, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (delay_ms != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_light_sensor_enabled(bool enable) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_LIGHT_SENSOR_ENABLE,
                                   0x00U, 0x00U, enable ? 0x01U : 0x00U);
}
...
mg58f18_radar_status_t mg58f18_radar_get_light_sensor_enabled(bool *enabled) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_LIGHT_SENSOR_ENABLE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (enabled != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_block_time_ms(uint32_t block_time_ms) {
  ...
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_u24(
      MG58F18_RADAR_CMD_SET_BLOCK_TIME, value, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  return (mg58f18_radar_frame_get_u24(&frame) == value)
 ...
 */
mg58f18_radar_status_t
mg58f18_radar_get_block_time_ms(uint32_t *block_time_ms) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_BLOCK_TIME, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
...
mg58f18_radar_status_t mg58f18_radar_set_active_level(bool high_active) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_ACTIVE_LEVEL, 0x00U,
                                   0x00U, high_active ? 0x01U : 0x00U);
}
...
mg58f18_radar_status_t mg58f18_radar_get_active_level(bool *high_active) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_ACTIVE_LEVEL, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (high_active != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_power_mode(bool normal_mode) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_POWER_MODE, 0x00U,
                                   0x00U, normal_mode ? 0x01U : 0x00U);
}
...
mg58f18_radar_status_t mg58f18_radar_get_power_mode(bool *normal_mode) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_POWER_MODE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (normal_mode != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_get_trigger_state(bool *triggered) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_TRIGGER_STATE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (triggered != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_get_light_environment(bool *is_night) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_LIGHT_ENVIRONMENT, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (is_night != NULL) {
 ...
 */
mg58f18_radar_status_t mg58f18_radar_get_firmware_version(uint8_t *major,
                                                          uint8_t *minor) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_FIRMWARE_VERSION, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (major != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_trigger_mode(bool single_trigger) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_TRIGGER_MODE, 0x00U,
                                   0x00U, single_trigger ? 0x01U : 0x00U);
}
/**
 * @brief 查询触发模式。
 */
mg58f18_radar_status_t mg58f18_radar_get_trigger_mode(bool *single_trigger) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_TRIGGER_MODE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
...
mg58f18_radar_status_t mg58f18_radar_set_power_step(uint8_t step) {
  ...
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_TX_POWER_STEP, 0x00U,
                                   0x00U, step);
}
/**
 * @brief 查询发射功率档位。
...
mg58f18_radar_status_t mg58f18_radar_get_power_step(uint8_t *step) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_TX_POWER_STEP, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (step != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_get_light_threshold(uint8_t *threshold) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_LIGHT_SENSOR_THRESHOLD, 0x00U, 0x00U, 0x00U,
      &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
...
mg58f18_radar_status_t mg58f18_radar_set_pwm_enabled(bool enable) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_PWM_ENABLE, 0x00U,
                                   0x00U, enable ? 0x01U : 0x00U);
}
/**
 * @brief 查询 PWM 使能状态。
 */
mg58f18_radar_status_t mg58f18_radar_get_pwm_enabled(bool *enable) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_PWM_ENABLE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (enable != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_pwm_duty_raw(uint16_t duty_raw) {
  ...
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_PWM_DUTY, 0x00U, data3,
                                   data4);
}
...
mg58f18_radar_status_t mg58f18_radar_get_pwm_duty_raw(uint16_t *duty_raw) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_PWM_DUTY, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (duty_raw != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_get_power_pulse_width(uint8_t *width) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_POWER_PULSE_WIDTH, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (width != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_set_sensing_mode(bool hand_mode) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SET_SENSING_MODE, 0x00U,
                                   0x00U, hand_mode ? 0x01U : 0x00U);
}
/**
 * @brief 查询响应模式。
 */
mg58f18_radar_status_t mg58f18_radar_get_sensing_mode(bool *hand_mode) {
  mg58f18_radar_frame_t frame;
  const mg58f18_radar_status_t status = mg58f18_radar_send_command_raw(
      MG58F18_RADAR_CMD_QUERY_SENSING_MODE, 0x00U, 0x00U, 0x00U, &frame);
  if (status != MG58F18_RADAR_STATUS_OK) {
    return status;
  }
  if (hand_mode != NULL) {
...
mg58f18_radar_status_t mg58f18_radar_save_settings(void) {
  return mg58f18_radar_expect_echo(MG58F18_RADAR_CMD_SAVE_SETTINGS, 0x00U,
                                   0x00U, 0x01U);
}
...
void mg58f18_radar_print_frame(const mg58f18_radar_frame_t *frame) {
  ...
  printf("[RADAR] cmd=0x%02X data2=0x%02X data3=0x%02X data4=0x%02X "
         "checksum=0x%02X\r\n",
         frame->command, frame->data2, frame->data3, frame->data4,
         frame->checksum);
  ...
  case MG58F18_RADAR_CMD_QUERY_DISTANCE_THRESHOLD: {
    const uint16_t threshold =
        (uint16_t)(((uint16_t)frame->data3 << 8) | frame->data4);
  ...
  case MG58F18_RADAR_CMD_QUERY_DELAY_TIME: {
    const uint32_t raw = ((uint32_t)frame->data2 << 16) |
                         ((uint32_t)frame->data3 << 8) | frame->data4;
    printf("        delay: %lu ms\r\n",
...
const char *mg58f18_radar_status_string(mg58f18_radar_status_t status) {
  switch (status) {
  case MG58F18_RADAR_STATUS_OK:
    return "ok";
  case MG58F18_RADAR_STATUS_NOT_INITIALISED:
    return "not_initialised";
  case MG58F18_RADAR_STATUS_BUSY:
    return "busy";
  case MG58F18_RADAR_STATUS_INVALID_ARGUMENT:
    return "invalid_argument";
  case MG58F18_RADAR_STATUS_TIMEOUT:
    return "timeout";
  case MG58F18_RADAR_STATUS_FRAME_ERROR:
    return "frame_error";
  case MG58F18_RADAR_STATUS_HAL_ERROR:
    return "hal_error";
  default:
    return "unknown";
  }
