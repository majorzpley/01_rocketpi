...
static uint8_t buzzer_test_apply(uint32_t frequency_hz, uint8_t duty_percent) {
  ...
  if (ticks_per_period > BUZZER_TEST_TIMER_MAX_PERIOD_TICKS) {
    prescaler = (uint32_t)((ticks_per_period +
                            BUZZER_TEST_TIMER_MAX_PERIOD_TICKS - 1ULL) /
                           BUZZER_TEST_TIMER_MAX_PERIOD_TICKS);
  ...
  printf("buzzer: start freq=%lu Hz duty=%u%% PSC=%lu ARR=%lu CCR=%lu\r\n",
         (unsigned long)s_last_freq_hz, (unsigned int)s_last_duty_percent,
         (unsigned long)(prescaler - 1U), (unsigned long)(auto_reload - 1U),
...
uint8_t buzzer_test_beep(uint32_t frequency_hz, uint8_t duty_percent,
                         uint32_t duration_ms) {
  const uint32_t duration =
      (duration_ms == 0U) ? BUZZER_TEST_DEFAULT_DURATION_MS : duration_ms;
...
uint8_t buzzer_test_play_sequence(const buzzer_test_note_t *notes,
                                  uint32_t length, uint32_t gap_ms) {
  const uint32_t gap =
      (gap_ms == 0U) ? BUZZER_TEST_DEFAULT_NOTE_GAP_MS : gap_ms;
