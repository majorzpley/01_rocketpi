 ...
 * @LastEditTime: 2026-02-02 13:08:09
 * @FilePath: /02_rocketpi_key_scan/src/main.c
 * @Description:此工程为rocketpi key按键测试工程
 * 不用客气，这是你应该谢的!
...
int main(void) {
  ...
  while (1) {
#if 0
    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0)) { //按下为高电平
      // todo 所有LED亮起
    ...
    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET) { // 检测到按下
      ...
      if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET) { // 仍然按下
        HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_1);                   //翻转LED
        HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_10 | GPIO_PIN_14);    //翻转LED
