 ...
 * @LastEditTime: 2026-02-06 11:34:53
 * @FilePath: /09_rocketpi_uart_control_led_cjson/src/main.c
 * @Description:使用CJSON库通过串口2来控制led亮灭
 * 不用客气，这是你应该谢的!
...
#define LED_ACTIVE_LOW 1 // 1表示低电平点亮，0表示高电平点亮
#if (LED_ACTIVE_LOW == 1)
...
enum { LED_TOKEN_COUNT = 3U, LED_TOTAL = 3U };
...
static bool console_parse_led_targets(const cJSON *root, LedCommand_t *cmd,
                                      const char **error_msg);
static bool console_parse_led_token(const char *token, LedCommand_t *cmd,
                                    const char **error_msg);
...
static bool console_parse_state_values(const cJSON *root, LedCommand_t *cmd,
                                       const char **error_msg);
static bool console_parse_state_token(const cJSON *item, LedCommand_t *cmd,
                                      const char **error_msg);
...
static bool console_state_from_string(const char *token, bool *value) {
  ...
  if (console_str_case_equal(ptr, "on") ||
      console_str_case_equal(ptr, "true") ||
      console_str_case_equal(ptr, "enable")) {
    *value = true;
    return true;
  }
  if (console_str_case_equal(ptr, "off") ||
      console_str_case_equal(ptr, "false") ||
...
static bool console_parse_state_token(const cJSON *item, LedCommand_t *cmd,
  ...
  cmd->led_state[cmd->state_count++] = value;
  return true;
}
...
static void console_apply_command(const LedCommand_t *cmd) {
  ...
  for (size_t i = 0U; i < cmd->led_count; ++i) {
    ...
    const bool desired = cmd->led_state[state_index];
  ...
  for (size_t i = 0U; i < cmd->led_count; ++i) {
    ...
    length +=
        snprintf(&buffer[length], (size_t)(sizeof(buffer) - (size_t)length),
                 "%u%s", cmd->led_state[state_index] ? 1U : 0U, delimiter);
  }
...
static void console_process_command_buffer(void) {
  ...
  if (!console_parse_state_values(root, &command, &error_msg)) {
    cJSON_Delete(root);
    console_report_error((error_msg != NULL) ? error_msg
                                             : "invalid state field");
    return;
  }
