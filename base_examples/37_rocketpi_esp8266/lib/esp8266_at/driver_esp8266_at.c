...
#define ESP8266_AT_BUILD_DEF(ID, NAME, LITERAL, T, Q, S, E)                    \
  [ID] = {                                                                     \
      ESP8266_AT_CMD_##NAME, LITERAL, (T) != 0, (Q) != 0, (S) != 0, (E) != 0},
...
static void esp8266_at_core_handle_line(esp8266_at_core_t *ctx,
                                        const char *line);
static void esp8266_at_core_process_prompt(esp8266_at_core_t *ctx);
static bool esp8266_at_parse_line_to_event(const char *line,
                                           esp8266_at_event_t *event);
static void esp8266_at_compact_from_event(esp8266_at_event_compact_t *dst,
                                          const esp8266_at_event_t *src);
static bool esp8266_at_event_from_compact(const esp8266_at_event_compact_t *src,
                                          esp8266_at_event_t *dst);
static void
esp8266_at_queue_restore_compact(esp8266_at_core_t *ctx,
                                 const esp8266_at_event_compact_t *events,
                                 size_t count);
static bool esp8266_at_queue_push(esp8266_at_core_t *ctx,
                                  const esp8266_at_event_t *event);
static bool esp8266_at_queue_pop(esp8266_at_core_t *ctx,
                                 esp8266_at_event_t *event);
...
static int esp8266_at_strncasecmp(const char *lhs, const char *rhs,
                                  size_t length);
static void esp8266_at_trim(char *str);
static void esp8266_at_copy_lower(char *dest, size_t dest_size,
                                  const char *source);
...
static esp8266_at_command_id_t
esp8266_at_match_command_by_prefix(const char *prefix);
static size_t esp8266_at_escape_field_length(const char *input);
static void esp8266_at_escape_field(const char *input, char *output,
                                    size_t output_size);
static esp8266_at_status_t esp8266_at_send_and_wait_internal(
    esp8266_at_command_id_t command, esp8266_at_command_mode_t mode,
    const char *arguments, uint32_t timeout_ms, bool expect_prompt,
    esp8266_at_event_t *terminal_event);
static bool esp8266_at_pop_event_by_prefix(const char *prefix,
                                           esp8266_at_event_t *out_event);
static const char *esp8266_at_event_type_name(esp8266_at_event_type_t type);
static esp8266_at_event_compact_t
    s_reset_event_stash[ESP8266_AT_EVENT_QUEUE_DEPTH];

const esp8266_at_command_def_t *
esp8266_at_get_command_def(esp8266_at_command_id_t command) {
...
const char *esp8266_at_status_string(esp8266_at_status_t status) {
  switch (status) {
  case ESP8266_AT_STATUS_OK:
    return "OK";
  case ESP8266_AT_STATUS_NOT_INITIALISED:
    return "not_initialised";
  case ESP8266_AT_STATUS_BUSY:
    return "busy";
  case ESP8266_AT_STATUS_INVALID_ARGUMENT:
    return "invalid_argument";
  case ESP8266_AT_STATUS_TIMEOUT:
    return "timeout";
  case ESP8266_AT_STATUS_HAL_ERROR:
    return "hal_error";
  case ESP8266_AT_STATUS_BUFFER_OVERFLOW:
    return "buffer_overflow";
  case ESP8266_AT_STATUS_ERROR_RESPONSE:
    return "error_response";
  case ESP8266_AT_STATUS_FAIL_RESPONSE:
    return "fail_response";
  case ESP8266_AT_STATUS_BUSY_RESPONSE:
    return "busy_response";
  case ESP8266_AT_STATUS_PROMPT:
    return "prompt";
  case ESP8266_AT_STATUS_UNEXPECTED_RESPONSE:
...
void esp8266_at_deinit(void) { memset(&s_core, 0, sizeof(s_core)); }

bool esp8266_at_is_ready(void) {
  return s_core.initialised && s_core.phase == ESP8266_AT_PHASE_IDLE &&
         !s_core.awaiting_reply;
}
...
esp8266_at_status_t esp8266_at_reset(uint32_t ready_timeout_ms) {
  ...
  const uint32_t timeout =
      (ready_timeout_ms != 0U) ? ready_timeout_ms : ESP8266_AT_RESET_TIMEOUT_MS;

  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_RST, ESP8266_AT_COMMAND_MODE_EXECUTE, NULL, timeout, false,
      NULL);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }
  ...
  while (esp8266_at_tick_elapsed(start, esp8266_at_interface_hw_get_tick()) <
         timeout) {
    esp8266_at_poll();
...
esp8266_at_status_t esp8266_at_disable_echo(bool disable) {
  ...
  return esp8266_at_send_and_wait_internal(
      command, ESP8266_AT_COMMAND_MODE_EXECUTE, NULL,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
}
...
esp8266_at_status_t esp8266_at_send_command(esp8266_at_command_id_t command,
                                            ...
                                            uint32_t timeout_ms,
                                            bool expect_prompt) {
  return esp8266_at_send_and_wait_internal(command, mode, arguments, timeout_ms,
                                           expect_prompt, NULL);
}

esp8266_at_status_t
esp8266_at_wait_for_completion(uint32_t timeout_ms,
                               esp8266_at_event_t *terminal_event) {
  if (!s_core.initialised) {
    return ESP8266_AT_STATUS_NOT_INITIALISED;
  }
  ...
  while (true) {
    ...
    if (esp8266_at_tick_elapsed(start, esp8266_at_interface_hw_get_tick()) >
        timeout) {
      s_core.awaiting_reply = false;
      s_core.phase = ESP8266_AT_PHASE_IDLE;
...
esp8266_at_status_t esp8266_at_send_raw_line(const char *line,
                                             uint32_t timeout_ms) {
  if (!s_core.initialised) {
    return ESP8266_AT_STATUS_NOT_INITIALISED;
  }
  ...
  const esp8266_at_status_t status = esp8266_at_interface_hw_send(
      (const uint8_t *)buffer, length + 2U, timeout_ms);
  return status;
}

esp8266_at_status_t esp8266_at_send_data(const uint8_t *payload, size_t length,
                                         uint32_t timeout_ms) {
  if (!s_core.initialised) {
    return ESP8266_AT_STATUS_NOT_INITIALISED;
  }
  ...
  s_core.command_timeout_ms =
      (timeout_ms != 0U) ? timeout_ms : ESP8266_AT_DEFAULT_TIMEOUT_MS;
...
esp8266_at_status_t esp8266_at_set_wifi_mode(uint8_t mode, bool store) {
  ...
  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_CWMODE, ESP8266_AT_COMMAND_MODE_SET, args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }
  ...
  if (store) {
    status = esp8266_at_send_and_wait_internal(
        ESP8266_AT_CMD_SYSSTORE, ESP8266_AT_COMMAND_MODE_SET, "1",
        ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
  }
...
esp8266_at_status_t esp8266_at_connect_ap(const char *ssid,
  ...
  (void)snprintf(args, sizeof(args), "\"%s\",\"%s\"", ssid_field,
                 password_field);

  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_CWJAP, ESP8266_AT_COMMAND_MODE_SET, args,
...
esp8266_at_status_t esp8266_at_disconnect_ap(void) {
  return esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_CWQAP, ESP8266_AT_COMMAND_MODE_EXECUTE, NULL,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
}
...
esp8266_at_status_t esp8266_at_query_ip_info(esp8266_at_ip_info_t *info) {
  ...
  esp8266_at_event_t event;
  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_CIPSTA, ESP8266_AT_COMMAND_MODE_QUERY, NULL,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }
  ...
  status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_CIPAP, ESP8266_AT_COMMAND_MODE_QUERY, NULL,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }
...
esp8266_at_status_t esp8266_at_mqtt_user_config(uint8_t link_id,
  ...
  esp8266_at_escape_field((client_id != NULL) ? client_id : "", client_field,
                          sizeof(client_field));
  esp8266_at_escape_field((username != NULL) ? username : "", username_field,
                          sizeof(username_field));
  esp8266_at_escape_field((password != NULL) ? password : "", password_field,
                          sizeof(password_field));

  char args[ESP8266_AT_MAX_LINE_LENGTH];
  (void)snprintf(args, sizeof(args), "%u,1,\"%s\",\"%s\",\"%s\",0,0,\"\"",
                 (unsigned int)link_id, client_field, username_field,
                 password_field);

  printf("[ESP8266][cmd] AT+MQTTUSERCFG=%s\r\n", args);

  return esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTUSERCFG, ESP8266_AT_COMMAND_MODE_SET, args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
}

esp8266_at_status_t esp8266_at_mqtt_connect(uint8_t link_id, const char *host,
                                            uint16_t port,
  ...
  (void)snprintf(cfg_args, sizeof(cfg_args), "%u,%u,%u,%u,0,0,\"\",\"\"",
                 (unsigned int)link_id, (unsigned int)keep_alive_s,
                 disable_clean_session, clean_session_flag);

  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTCONNCFG, ESP8266_AT_COMMAND_MODE_SET, cfg_args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }
  ...
  (void)snprintf(conn_args, sizeof(conn_args), "%u,\"%s\",%u,0",
                 (unsigned int)link_id, host_field, (unsigned int)port);

  printf("[ESP8266][cmd] AT+MQTTCONN=%s\r\n", conn_args);

  return esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTCONN, ESP8266_AT_COMMAND_MODE_SET, conn_args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS * 3U, false, NULL);
}

esp8266_at_status_t esp8266_at_mqtt_publish(uint8_t link_id, const char *topic,
  ...
  if (topic_escaped_length >= sizeof(topic_field) ||
    ...
    return esp8266_at_mqtt_publish_raw(link_id, topic, (const uint8_t *)payload,
                                       payload_plain_length, qos, retain);
  }
  ...
  char args[ESP8266_AT_MAX_LINE_LENGTH];
  const int arg_length = snprintf(
      args, sizeof(args), "%u,\"%s\",\"%s\",%u,%u", (unsigned int)link_id,
      topic_field, payload_field, (unsigned int)qos, retain ? 1U : 0U);

  const esp8266_at_command_def_t *cmd_def =
      esp8266_at_get_command_def(ESP8266_AT_CMD_MQTTPUB);
  ...
  if (arg_length < 0 || (size_t)arg_length >= sizeof(args) ||
  ...
  return esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTPUB, ESP8266_AT_COMMAND_MODE_SET, args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS * 2U, false, NULL);
}
...
esp8266_at_status_t esp8266_at_mqtt_publish_raw(uint8_t link_id,
  ...
  char args[ESP8266_AT_MAX_LINE_LENGTH];
  (void)snprintf(args, sizeof(args), "%u,\"%s\",%u,%u,%u",
                 (unsigned int)link_id, topic_field, (unsigned int)length,
                 (unsigned int)qos, retain ? 1U : 0U);

  esp8266_at_status_t status = esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTPUBRAW, ESP8266_AT_COMMAND_MODE_SET, args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, true, NULL);
  if (status != ESP8266_AT_STATUS_PROMPT) {
    return status;
  }

  status =
      esp8266_at_send_data(payload, length, ESP8266_AT_DEFAULT_TIMEOUT_MS * 2U);
  if (status != ESP8266_AT_STATUS_OK) {
    return status;
  }

  status =
      esp8266_at_wait_for_completion(ESP8266_AT_DEFAULT_TIMEOUT_MS * 2U, NULL);
  return status;
}
...
esp8266_at_status_t esp8266_at_mqtt_disconnect(uint8_t link_id) {
  ...
  const char args[] = "0";
  return esp8266_at_send_and_wait_internal(
      ESP8266_AT_CMD_MQTTCLEAN, ESP8266_AT_COMMAND_MODE_SET, args,
      ESP8266_AT_DEFAULT_TIMEOUT_MS, false, NULL);
}
...
size_t esp8266_at_pending_event_count(void) { return s_core.queue.count; }
...
void esp8266_at_poll(void) {
  ...
  if (esp8266_at_tick_elapsed(s_core.command_start_tick, now) >
      s_core.command_timeout_ms) {
    s_core.awaiting_reply = false;
    s_core.phase = ESP8266_AT_PHASE_IDLE;
    s_core.last_status = ESP8266_AT_STATUS_TIMEOUT;
...
void esp8266_at_debug_print_events(bool keep_events) {
  ...
  /* Current implementation does not print; it effectively clears the queue
   * unless asked to keep events. */
  if (!keep_events) {
...
static esp8266_at_status_t esp8266_at_send_and_wait_internal(
  ...
  const size_t literal_length =
      esp8266_at_strnlen(def->literal, sizeof(tx_buffer) - 3U);
  ...
  if (mode == ESP8266_AT_COMMAND_MODE_QUERY) {
    ...
    const size_t arg_len =
        esp8266_at_strnlen(arguments, sizeof(tx_buffer) - offset - 3U);
    memcpy(&tx_buffer[offset], arguments, arg_len);
    offset += arg_len;
  } else if (mode == ESP8266_AT_COMMAND_MODE_EXECUTE && arguments != NULL &&
             arguments[0] != '\0') {
    const size_t arg_len =
        esp8266_at_strnlen(arguments, sizeof(tx_buffer) - offset - 3U);
    memcpy(&tx_buffer[offset], arguments, arg_len);
  ...
  const esp8266_at_status_t send_status = esp8266_at_interface_hw_send(
      (const uint8_t *)tx_buffer, offset, timeout_ms);
  ...
  s_core.command_start_tick = esp8266_at_interface_hw_get_tick();
  s_core.last_status =
      expect_prompt ? ESP8266_AT_STATUS_PROMPT : ESP8266_AT_STATUS_TIMEOUT;
  s_core.last_terminal_event_valid = false;
...
static void esp8266_at_core_process_byte(esp8266_at_core_t *ctx, uint8_t byte) {
  ...
  if (ctx->phase == ESP8266_AT_PHASE_WAITING_PROMPT &&
      byte == (uint8_t)ESP8266_AT_PROMPT_CHAR && ctx->line.length == 0U) {
    esp8266_at_core_process_prompt(ctx);
    return;
  }

  if (ctx->phase == ESP8266_AT_PHASE_READY_FOR_DATA && ctx->line.length == 0U &&
      byte == ' ') {
    return;
  }
...
esp8266_at_queue_restore_compact(esp8266_at_core_t *ctx,
  ...
  for (size_t i = 0U; i < count; ++i) {
    if (ctx->queue.count >= ESP8266_AT_EVENT_QUEUE_DEPTH) {
      ctx->queue.head =
          (uint8_t)((ctx->queue.head + 1U) % ESP8266_AT_EVENT_QUEUE_DEPTH);
      ctx->queue.count--;
    }

    ctx->queue.events[ctx->queue.tail] = events[i];
    ctx->queue.tail =
        (uint8_t)((ctx->queue.tail + 1U) % ESP8266_AT_EVENT_QUEUE_DEPTH);
    ctx->queue.count++;
  }
}
...
static const char *esp8266_at_event_type_name(esp8266_at_event_type_t type) {
  switch (type) {
  case ESP8266_AT_EVENT_TYPE_INFO:
    return "info";
  case ESP8266_AT_EVENT_TYPE_INDICATION:
    return "ind";
  case ESP8266_AT_EVENT_TYPE_RESPONSE:
    return "resp";
  case ESP8266_AT_EVENT_TYPE_OK:
    return "ok";
  case ESP8266_AT_EVENT_TYPE_ERROR:
  ...
  case ESP8266_AT_EVENT_TYPE_BUSY:
    return "busy";
  case ESP8266_AT_EVENT_TYPE_PROMPT:
    return "prompt";
  case ESP8266_AT_EVENT_TYPE_SEND_OK:
    return "send_ok";
  case ESP8266_AT_EVENT_TYPE_SEND_FAIL:
...
static void esp8266_at_parse_ip_descriptor(const esp8266_at_event_t *event,
  ...
  const char *payload =
      (event->payload[0] != '\0') ? event->payload : event->raw_line;
