...
#define ESP8266_AT_TEST_WIFI_SSID "ASUS"
#endif
#ifndef ESP8266_AT_TEST_WIFI_PASSWORD
#define ESP8266_AT_TEST_WIFI_PASSWORD "12345wyx"
#endif
...
static void esp8266_at_test_log_status(const char *label,
                                       esp8266_at_status_t status) {
  printf("[ESP8266][%s] %s\r\n", label, esp8266_at_status_string(status));
}

static const char *
esp8266_at_test_event_type_string(esp8266_at_event_type_t type) {
  switch (type) {
...
static void esp8266_at_test_print_event(const esp8266_at_event_t *event) {
  ...
  printf("[ESP8266][event][%s] %s",
         esp8266_at_test_event_type_string(event->type), event->raw_line);
...
void esp8266_at_test_run(void) {
  ...
  status = esp8266_at_send_command(ESP8266_AT_CMD_AT,
                                   ESP8266_AT_COMMAND_MODE_EXECUTE, NULL,
  ...
  status =
      esp8266_at_send_command(ESP8266_AT_CMD_CMD, ESP8266_AT_COMMAND_MODE_QUERY,
                              NULL, ESP8266_AT_DEFAULT_TIMEOUT_MS * 2U, false);
  esp8266_at_test_log_status("AT+CMD?", status);
  esp8266_at_test_drain_events(100U);

  const bool wifi_configured =
      !esp8266_at_test_is_placeholder(ESP8266_AT_TEST_WIFI_SSID,
                                      "YOUR_WIFI_SSID") &&
      !esp8266_at_test_is_placeholder(ESP8266_AT_TEST_WIFI_PASSWORD,
                                      "YOUR_WIFI_PASSWORD");
  if (wifi_configured) {
    ...
    status =
        esp8266_at_connect_ap(ESP8266_AT_TEST_WIFI_SSID,
                              ESP8266_AT_TEST_WIFI_PASSWORD, 20000U, false);
    ...
    if (status == ESP8266_AT_STATUS_OK) {
      printf("        STA ip=%s gateway=%s netmask=%s\r\n", ip_info.station_ip,
             ip_info.station_gateway, ip_info.station_netmask);
      printf("        AP ip=%s gateway=%s netmask=%s\r\n", ip_info.softap_ip,
             ip_info.softap_gateway, ip_info.softap_netmask);
    }
    esp8266_at_test_drain_events(100U);
  } else {
    printf("[ESP8266][wifi] skipped (define "
           "ESP8266_AT_TEST_WIFI_SSID/PASSWORD)\r\n");
  }

  const bool mqtt_configured =
      !esp8266_at_test_is_placeholder(ESP8266_AT_TEST_MQTT_BROKER,
                                      "YOUR_MQTT_BROKER") &&
      !esp8266_at_test_is_placeholder(ESP8266_AT_TEST_MQTT_CLIENT_ID,
                                      "YOUR_MQTT_CLIENT_ID");
  if (mqtt_configured) {
    const char *mqtt_host = ESP8266_AT_TEST_MQTT_BROKER;

    status = esp8266_at_mqtt_user_config(0U, ESP8266_AT_TEST_MQTT_CLIENT_ID,
                                         ESP8266_AT_TEST_MQTT_USERNAME,
                                         ESP8266_AT_TEST_MQTT_PASSWORD);
    esp8266_at_test_log_status("MQTTUSERCFG", status);
    esp8266_at_test_drain_events(50U);

    status = esp8266_at_mqtt_connect(0U, mqtt_host, ESP8266_AT_TEST_MQTT_PORT,
                                     120U, true);
    ...
    printf(
        "[ESP8266][mqtt] skipped (define ESP8266_AT_TEST_MQTT_* macros)\r\n");
  }
