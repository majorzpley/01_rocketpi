...
#define LFS_FLASH_PORT_ERROR_FLAGS                                             \
...
static int lfs_flash_port_prog(const struct lfs_config *cfg, lfs_block_t block,
                               ...
                               lfs_size_t size);
static int lfs_flash_port_erase(const struct lfs_config *cfg,
                                lfs_block_t block);
...
static bool lfs_flash_port_range_valid(lfs_block_t block, lfs_off_t off,
                                       lfs_size_t size) {
...
lfs_flash_port_program(uint32_t address, const uint8_t *data, size_t size) {
...
static int lfs_flash_port_prog(const struct lfs_config *cfg, lfs_block_t block,
  ...
  HAL_StatusTypeDef status =
      lfs_flash_port_program(address, (const uint8_t *)buffer, size);
  HAL_FLASH_Lock();
...
int lfs_flash_port_unmount(lfs_t *lfs) { return lfs_unmount(lfs); }