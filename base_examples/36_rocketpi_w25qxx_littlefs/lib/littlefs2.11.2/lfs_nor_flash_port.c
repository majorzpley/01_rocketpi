...
static bool lfs_nor_flash_range_valid(lfs_block_t block, lfs_off_t off,
                                      lfs_size_t size) {
...
static int lfs_nor_flash_hw_init(void) {
  ...
  DRIVER_W25QXX_LINK_SPI_QSPI_INIT(&g_w25qxx_handle,
                                   w25qxx_interface_spi_qspi_init);
  DRIVER_W25QXX_LINK_SPI_QSPI_DEINIT(&g_w25qxx_handle,
                                     w25qxx_interface_spi_qspi_deinit);
  DRIVER_W25QXX_LINK_SPI_QSPI_WRITE_READ(&g_w25qxx_handle,
  ...
  DRIVER_W25QXX_LINK_DEBUG_PRINT(&g_w25qxx_handle,
...
static int lfs_nor_flash_prog(const struct lfs_config *cfg, lfs_block_t block,
  ...
  while (size > 0U) {
    ...
    if (w25qxx_page_program(&g_w25qxx_handle, address, (uint8_t *)data,
                            (uint16_t)chunk) != 0U) {
      return LFS_ERR_IO;
    }
...
static int lfs_nor_flash_erase(const struct lfs_config *cfg,
                               lfs_block_t block) {
  (void)cfg;
