 ...
 * @LastEditTime: 2026-03-10 15:30:04
 * @FilePath: /01_rocketpi_bootloader_iap/bootloader/src/main.c
 * @Description:此工程为rocketpi bootloader iap bootloader工程
 * 不用客气，这是你应该谢的!
...
int main(void) {
  ...
  // todo 是否需要进入boot模式：5S内PA0是否按下
  bool enterUpgrade = Bootloader_ShouldEnterUpgradeMode();
  // Bootloader_JumpToApplication();
  // todo 若没有被按下，判断地址是否合法
  if (!enterUpgrade) {
    if (Bootloader_IsApplicationValid()) {
    bootApp:
      // todo 地址合法，跳转至app处
      printf(
          "[BOOT] valid application found, quit boot mode and enter app.\r\n");
      Bootloader_JumpToApplication();
    } else {
      // todo 地址非法，打印日志
  ...
  // todo 若按下了PA0按键或者地址非法，则跳转至boot模式，接收APP分区固件
  if (enterUpgrade) {
...
static void Bootloader_JumpToApplication(void) {
  ...
  Bootloader_DeInitPeripherals(); //注销外设
...
static bool Bootloader_IsKeyPressed(void) {
  return HAL_GPIO_ReadPin(BOOT_KEY_GPIO_PORT, BOOT_KEY_GPIO_PIN) ==
         BOOT_KEY_PRESSED_STATE;
}
...
static void Bootloader_RunUpgradeMode(void) {
  ...
  while (1) {
    ...
    printf("[BOOT] Firmware invalid or upgrade aborted, waiting for another "
           "transfer.\r\n");
    HAL_Delay(500U);
  }
}
