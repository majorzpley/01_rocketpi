...
static void app_build_mqtt_identifiers(void) {
  ...
  (void)snprintf(s_app.mqtt_sensor_topic, sizeof(s_app.mqtt_sensor_topic),
                 "%s/sensors/%s/aht30", APP_MQTT_BASE_TOPIC, s_app.device_id);

  (void)snprintf(s_app.mqtt_control_topic, sizeof(s_app.mqtt_control_topic),
                 "%s/actuators/%s/led/cmd", APP_MQTT_BASE_TOPIC,
                 s_app.device_id);

  (void)snprintf(s_app.mqtt_motor_topic, sizeof(s_app.mqtt_motor_topic),
                 "%s/actuators/%s/motor/cmd", APP_MQTT_BASE_TOPIC,
                 s_app.device_id);

  (void)snprintf(s_app.mqtt_buzzer_topic, sizeof(s_app.mqtt_buzzer_topic),
                 "%s/actuators/%s/buzzer/cmd", APP_MQTT_BASE_TOPIC,
                 s_app.device_id);
}
...
static void app_lcd_show_status(void) {
  ...
  (void)snprintf(line, sizeof(line), "WiFi:%s",
                 s_app.wifi_ready ? "OK" : "OFF");
  ST7789_ShowString(10, 60, line, Font16x24, s_app.wifi_ready ? GREEN : YELLOW,
                    BLACK);

  (void)snprintf(line, sizeof(line), "MQTT:%s",
                 s_app.mqtt_ready ? "OK" : "WAIT");
  ST7789_ShowString(10, 100, line, Font16x24, s_app.mqtt_ready ? GREEN : YELLOW,
                    BLACK);
...
static void app_lcd_show_sensor(int16_t temp10, uint16_t hum10) {
  ...
  if (temp10 == INT16_MIN || hum10 == UINT16_MAX) {
    ...
    (void)snprintf(line, sizeof(line), "T:%d.%01dC", temp10 / 10,
                   abs(temp10 % 10));
    ST7789_ShowString(10, 40, line, Font16x24, WHITE, BLACK);

    (void)snprintf(line, sizeof(line), "H:%u.%01u%%",
                   (unsigned int)(hum10 / 10), (unsigned int)(hum10 % 10));
...
static bool app_get_bool_field(const cJSON *root, const char *key,
                               bool *value) {
...
static esp8266_at_status_t app_mqtt_subscribe_topic(const char *topic,
  ...
  esp8266_at_status_t status = esp8266_at_send_command(
...
static void app_apply_motor_command(const cJSON *root) {
  ...
  printf("[APP][cmd] motor %s speed=%d\r\n",
         (dir == MOTOR_L9110_DIR_FORWARD) ? "forward" : "reverse", speed_value);
}
...
static void app_handle_control_command(const char *topic, const char *json,
                                       size_t length) {
  /* Supported MQTT commands (topics: led/motor/buzzer control topics)
   * LED:
   * - {"cmd":"led","id":"b","state":1}              // single: b/g/p or
   * blue/green/pink
...
static esp8266_at_status_t app_publish_aht30(float temperature_c,
                                             float humidity_pct) {
  ...
  (void)snprintf(temperature_raw, sizeof(temperature_raw), "%.1f",
                 (double)temperature_rounded);
  (void)snprintf(humidity_raw, sizeof(humidity_raw), "%.1f",
                 (double)humidity_rounded);
  ...
  if (payload != NULL) {
    ...
    status = esp8266_at_mqtt_publish_raw(0U, s_app.mqtt_sensor_topic,
                                         (const uint8_t *)payload, payload_len,
                                         APP_MQTT_QOS, false);
    if (status != ESP8266_AT_STATUS_OK) {
      printf("[APP][mqtt_pub] payload_len=%u payload=%s\r\n",
             (unsigned int)payload_len, payload);
    }
    free(payload);
  }
...
void App_Init(void) {
  ...
  motor_l9110_init();
  (void)buzzer_test_stop();
  (void)buzzer_test_beep(APP_BUZZER_DEFAULT_FREQ_HZ, APP_BUZZER_DUTY_PERCENT,
                         120U);
  HAL_Delay(80U);
  (void)buzzer_test_beep(APP_BUZZER_DEFAULT_FREQ_HZ, APP_BUZZER_DUTY_PERCENT,
                         120U);
...
void App_Loop(void) {
  ...
  if (status == HAL_OK) {
    const esp8266_at_status_t pub_status =
        app_publish_aht30(temperature, humidity);
    ...
    printf("[APP] aht30 T=%d.%01dC RH=%d.%01d%%\r\n", temp10 / 10,
           abs(temp10 % 10), hum10 / 10, hum10 % 10);

    if (s_app.lcd_ready &&
        (s_app.lcd_showing_status || temp10 != s_app.last_temp10 ||
