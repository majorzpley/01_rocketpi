...
static void esp8266_at_interface_trace_store(const uint8_t *data,
                                             uint16_t size) {
...
static bool esp8266_at_interface_start_rx(void) {
  ...
  HAL_StatusTypeDef status = HAL_UARTEx_ReceiveToIdle_DMA(
      s_hal.uart, s_hal.rx_buffer, sizeof(s_hal.rx_buffer));
  ...
  if (status == HAL_BUSY) {
    ...
    status = HAL_UARTEx_ReceiveToIdle_DMA(s_hal.uart, s_hal.rx_buffer,
                                          sizeof(s_hal.rx_buffer));
...
esp8266_at_status_t esp8266_at_interface_hw_send(const uint8_t *data,
                                                 size_t length,
                                                 uint32_t timeout_ms) {
  if (!s_hal.initialised || s_hal.uart == NULL || data == NULL ||
      length == 0U) {
...
uint32_t esp8266_at_interface_hw_get_tick(void) { return HAL_GetTick(); }
