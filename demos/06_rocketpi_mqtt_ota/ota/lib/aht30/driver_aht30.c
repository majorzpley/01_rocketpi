...
static soft_i2c_status_t aht30_gpio_write(void *ctx,
...
static HAL_StatusTypeDef aht30_bus_transmit(const uint8_t *data, size_t size) {
  soft_i2c_status_t status =
      soft_i2c_master_transmit(&s_aht30_bus, AHT30_I2C_ADDRESS, data, size);
...
static HAL_StatusTypeDef aht30_bus_receive(uint8_t *data, size_t size) {
  soft_i2c_status_t status =
      soft_i2c_master_receive(&s_aht30_bus, AHT30_I2C_ADDRESS, data, size);
  if (status == SOFT_I2C_STATUS_TIMEOUT) {
    return HAL_TIMEOUT;
  }
  return (status == SOFT_I2C_STATUS_OK) ? HAL_OK : HAL_ERROR;
...
static HAL_StatusTypeDef aht30_bus_init(void) { return HAL_OK; }

static HAL_StatusTypeDef aht30_bus_transmit(const uint8_t *data, size_t size) {
  return HAL_I2C_Master_Transmit(&AHT30_I2C_HANDLE, AHT30_I2C_ADDRESS,
                                 (uint8_t *)data, size, HAL_MAX_DELAY);
}

static HAL_StatusTypeDef aht30_bus_receive(uint8_t *data, size_t size) {
  return HAL_I2C_Master_Receive(&AHT30_I2C_HANDLE, AHT30_I2C_ADDRESS, data,
                                size, HAL_MAX_DELAY);
}
...
static void aht30_convert_samples(const uint8_t raw[6], float *temperature_c,
                                  float *humidity_pct) {
  uint32_t raw_humidity = ((uint32_t)raw[1] << 12) | ((uint32_t)raw[2] << 4) |
                          (uint32_t)(raw[3] >> 4);
