...
static bool app_has_custom_device_id(void) {
  return (APP_DEVICE_ID[0] != '\0' && strcmp(APP_DEVICE_ID, "AUTO") != 0 &&
...
static void app_build_mqtt_identifiers(void) {
  ...
  if (app_has_custom_client_id()) {
    strncpy(s_app.mqtt_client_id, APP_MQTT_CLIENT_ID,
            sizeof(s_app.mqtt_client_id) - 1U);
    s_app.mqtt_client_id[sizeof(s_app.mqtt_client_id) - 1U] = '\0';
  } else {
    (void)snprintf(s_app.mqtt_client_id, sizeof(s_app.mqtt_client_id), "%s%s",
                   APP_MQTT_CLIENT_ID_PREFIX, s_app.device_id);
  }
}
...
static void app_sha256_transform(app_sha256_ctx_t *ctx,
  ...
  for (size_t i = 16U; i < 64U; ++i) {
    const uint32_t s0 = app_rotr32(m[i - 15U], 7U) ^
                        app_rotr32(m[i - 15U], 18U) ^ (m[i - 15U] >> 3U);
    const uint32_t s1 = app_rotr32(m[i - 2U], 17U) ^
                        app_rotr32(m[i - 2U], 19U) ^ (m[i - 2U] >> 10U);
  ...
  for (size_t i = 0U; i < 64U; ++i) {
    ...
    const uint32_t s1 =
        app_rotr32(e, 6U) ^ app_rotr32(e, 11U) ^ app_rotr32(e, 25U);
    const uint32_t s0 =
        app_rotr32(a, 2U) ^ app_rotr32(a, 13U) ^ app_rotr32(a, 22U);
...
static void app_sha256_update(app_sha256_ctx_t *ctx, const uint8_t *data,
                              size_t len) {
...
static void app_bytes_to_hex(const uint8_t *bytes, size_t length, char *out,
                             size_t out_size) {
...
static void app_log_payload_preview(const char *label, const char *data,
                                    size_t length) {
  ...
  if (length <= head_len) {
    printf("[OTA][%s] len=%lu head=%s\r\n", label, (unsigned long)length, head);
    return;
  }
...
static bool app_is_image_valid(void) {
  ...
  const bool sp_ok =
      (app_sp >= SRAM_START_ADDRESS) && (app_sp <= SRAM_END_ADDRESS);
  const bool reset_ok =
      (app_reset >= APP_START_ADDRESS) && (app_reset <= APP_END_ADDRESS);
...
static void app_build_ota_topic(char *buffer, size_t size, const char *suffix) {
  (void)snprintf(buffer, size, "%s/ota/%s/%s", APP_MQTT_BASE_TOPIC,
                 s_app.device_id, suffix);
}

static esp8266_at_status_t app_mqtt_subscribe_topic(const char *topic,
                                                    const char *label) {
  ...
  (void)snprintf(sub_args, sizeof(sub_args), "%u,\"%s\",%u", 0U, topic,
                 (unsigned int)APP_MQTT_QOS);

  const esp8266_at_status_t status = esp8266_at_send_command(
      ESP8266_AT_CMD_MQTTSUB, ESP8266_AT_COMMAND_MODE_SET, sub_args,
...
static void app_mqtt_try_reconnect(void) {
  ...
  const esp8266_at_status_t status =
      esp8266_at_mqtt_connect(0U, APP_MQTT_BROKER, APP_MQTT_PORT, 120U, true);
...
static bool app_parse_mqtt_subrecv(const char *payload, char *topic,
                                   size_t topic_size, const char **data_ptr,
                                   size_t *data_len) {
...
static bool app_base64_decode(const char *input, size_t input_len,
  ...
  for (size_t i = 0U; i < length; ++i) {
    if (ptr[i] != 0xFFU) {
      printf("[OTA] flash not blank at 0x%08lx value=0x%02x\r\n",
             (unsigned long)(address + i), (unsigned int)ptr[i]);
      return false;
    }
  }
  return true;
...
static bool app_flash_prepare(void) {
  printf("[OTA] flash region base=0x%08lx size=%lu\r\n",
         (unsigned long)OTA_FLASH_BASE, (unsigned long)OTA_FLASH_SIZE);
  HAL_FLASH_Unlock();
  ...
  if (status != HAL_OK) {
    printf("[OTA] flash erase failed, err=0x%08lx code=0x%08lx\r\n",
           (unsigned long)error_sector, (unsigned long)HAL_FLASH_GetError());
    HAL_FLASH_Lock();
    return false;
  }
...
static bool app_flash_write(uint32_t address, const uint8_t *data,
                            size_t length) {
  ...
  for (size_t offset = 0U; offset < length; ++offset) {
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_BYTE, write_addr, data[offset]) !=
        HAL_OK) {
      printf("[OTA] flash program failed at 0x%08lx code=0x%08lx\r\n",
             (unsigned long)write_addr, (unsigned long)HAL_FLASH_GetError());
      return false;
    }
    write_addr += 1U;
  }
  ...
  if (length > 0U) {
    ...
    if (first != data[0] || last != data[length - 1U]) {
      printf("[OTA] flash verify mismatch addr=0x%08lx first=0x%02x "
             "last=0x%02x\r\n",
             (unsigned long)address, (unsigned int)first, (unsigned int)last);
      return false;
    }
  }
...
static esp8266_at_status_t app_mqtt_publish_json(const char *topic,
                                                 cJSON *root) {
  ...
  if (payload != NULL) {
    ...
    status = esp8266_at_mqtt_publish_raw(0U, topic, (const uint8_t *)payload,
                                         payload_len, APP_MQTT_QOS, false);
    printf("[OTA] mqtt pub topic=%s len=%lu status=%s\r\n", topic,
           (unsigned long)payload_len, esp8266_at_status_string(status));
    free(payload);
  }
  return status;
}
...
static void app_send_state(uint32_t progress, const char *phase,
                           const char *status) {
  char topic[96];
...
static void app_handle_offer(const cJSON *root) {
  ...
  s_app.total_chunks =
      (s_app.total_size + s_app.chunk_size - 1U) / s_app.chunk_size;
  s_app.expected_index = 0U;
  s_app.bytes_written = 0U;
  s_app.last_progress = 0U;
  ...
  if (s_app.total_size == 0U || s_app.total_size > OTA_FLASH_SIZE) {
    printf("[OTA] image size overflow: %lu (limit %lu)\r\n",
           (unsigned long)s_app.total_size, (unsigned long)OTA_FLASH_SIZE);
  ...
  if (cJSON_IsString(version)) {
    strncpy(s_app.offer_version, version->valuestring,
            sizeof(s_app.offer_version) - 1U);
  }
  if (cJSON_IsString(sha256)) {
    strncpy(s_app.offer_sha256, sha256->valuestring,
            sizeof(s_app.offer_sha256) - 1U);
  }

  printf("[OTA] offer size=%lu chunk=%lu total=%lu version=%s\r\n",
         (unsigned long)s_app.total_size, (unsigned long)s_app.chunk_size,
...
static void app_handle_chunk_json(const char *json, size_t length) {
  ...
  if (chunk_index != s_app.expected_index) {
    printf("[OTA] chunk out of order: got=%lu expect=%lu\r\n",
           (unsigned long)chunk_index, (unsigned long)s_app.expected_index);
  ...
  if (!app_base64_decode(data_str, data_len, chunk_buffer, sizeof(chunk_buffer),
                         &out_len)) {
  ...
  if (cJSON_IsNumber(size) && out_len != (size_t)size->valuedouble) {
    printf("[OTA] size mismatch: payload=%lu expected=%lu\r\n",
           (unsigned long)out_len, (unsigned long)size->valuedouble);
  }

  printf("[OTA] chunk index=%lu offset=%lu size=%lu\r\n",
         (unsigned long)chunk_index, (unsigned long)chunk_offset,
  ...
  if (s_app.bytes_written >= s_app.total_size ||
      s_app.expected_index >= s_app.total_chunks) {
...
static void app_handle_chunk_fragment(const char *data, size_t length) {
  ...
  const size_t trimmed_len =
      app_trim_tail_length(s_chunk_json_buffer, s_chunk_json_len);
  if (trimmed_len == 0U) {
    return;
  }
  if (s_chunk_json_buffer[0] != '{' ||
      s_chunk_json_buffer[trimmed_len - 1U] != '}') {
...
static void app_process_mqtt_event(const esp8266_at_event_t *event) {
  ...
  if (strcmp(event->prefix, "+MQTTSUBRECV") == 0) {
    ...
    size_t data_len = 0U;
    if (!app_parse_mqtt_subrecv(event->payload, topic, sizeof(topic), &data,
                                &data_len)) {
    ...
    if (data_len > available) {
      printf("[OTA] mqtt payload truncated len=%lu available=%lu\r\n",
             (unsigned long)data_len, (unsigned long)available);
      data_len = available;
    }
...
void App_Init(void) {
  ...
  if (wifi_configured) {
    ...
    status =
        esp8266_at_connect_ap(APP_WIFI_SSID, APP_WIFI_PASSWORD, 20000U, false);
  ...
  if (mqtt_configured) {
    status = esp8266_at_mqtt_user_config(0U, s_app.mqtt_client_id,
