 ...
 * @Date: 2026-03-11 13:10:04
 * @LastEditors: majorzpley wyx1214844230@outlook.com
 * @LastEditTime: 2026-03-11 13:40:57
 * @FilePath: /02_don't_approach_me_alarm/src/app.c
 * @Description:
 * 不用客气，这是你应该谢的!
...
#define APP_BEEP_DUTY_PERCENT                                                  \
  BUZZER_TEST_DEFAULT_DUTY_PERCENT /* 蜂鸣占空比                          \
                                    */
...
/* 停止蜂鸣器输出 */
static void app_buzzer_stop_tone(void) { (void)buzzer_test_stop(); }
...
static void app_handle_ir_command(uint8_t command) {
  ...
  case APP_IR_KEY_DISTANCE_FARTHER: {
    uint32_t candidate =
        (s_distance_threshold > APP_DISTANCE_STEP)
  ...
  case APP_IR_KEY_SAVE: {
    ...
    uint16_t dist = 0U;
    uint32_t hold_ms = 0U;
    const mg58f18_radar_status_t dist_status =
        mg58f18_radar_get_distance_threshold(&dist);
    const mg58f18_radar_status_t hold_status =
        mg58f18_radar_get_delay_ms(&hold_ms);
    ...
    printf("[APP][RADAR] after save: distance=%u hold=%lu ms (dist:%s "
           "hold:%s)\r\n",
           (unsigned int)s_distance_threshold, (unsigned long)s_delay_ms,
...
static void app_apply_distance_threshold(uint16_t new_value) {
  ...
  const mg58f18_radar_status_t status =
      mg58f18_radar_set_distance_threshold(new_value);
...
void app_init(void) {
  ...
  const mg58f18_radar_status_t io_status =
      mg58f18_radar_read_io(&s_radar_io_state);
