...
void Spectrum_Init(uint32_t sample_rate_hz) {
  ...
  const float bin_width_hz =
      ((float)spectrum_sample_rate * 0.5f) / (float)SPECTRUM_BIN_COUNT;
...
void Spectrum_UpdateFromInterleaved(const int16_t *samples, size_t sample_count,
                                    uint32_t channels) {
...
uint32_t Spectrum_GetBinCount(void) { return SPECTRUM_BIN_COUNT; }
...
uint16_t Spectrum_GetMaxHeight(void) { return SPECTRUM_MAX_HEIGHT; }