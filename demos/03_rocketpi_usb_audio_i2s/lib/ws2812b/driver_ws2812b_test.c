 ...
 */
static void ws_test_color_wheel(uint8_t pos, uint8_t *r, uint8_t *g,
                                uint8_t *b) {
  if (pos < 85U) {
    *r = (uint8_t)(pos * 3U);
  ...
  while (!ws_test_should_stop(start_tick, duration_ms)) {
    for (uint16_t offset = 0; offset < 255U; ++offset) {
      for (uint16_t led = 0; led < count; ++led) {
        uint8_t r, g, b;
        ws_test_color_wheel((uint8_t)((led * 256U / count + offset) & 0xFFU),
                            &r, &g, &b);
 ...
 * @param duration_ms Total runtime (0 = infinite loop).
 */
void ws2812b_test_gradient_wipe(uint8_t start_r, uint8_t start_g,
                                uint8_t start_b, uint8_t end_r, uint8_t end_g,
                                uint8_t end_b, uint32_t step_delay_ms,
                                uint32_t duration_ms) {
  const uint16_t count = ws2812b_get_led_count();
  if (count == 0U) {
    return;
