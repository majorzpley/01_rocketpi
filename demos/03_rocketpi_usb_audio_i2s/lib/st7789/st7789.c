...
/* 需在 st7789.h 或工程配置里定义：
   ...
   - 控制脚：ST7789_CS_LOW/HIGH(), ST7789_DC_LOW/HIGH(), ST7789_RST_LOW/HIGH(),
   ST7789_BL_LOW/HIGH()
*/
#ifndef ST7789_SPI
...
static void ST7789_SetWindow(uint16_t x0, uint16_t y0, uint16_t x1,
                             uint16_t y1) {
...
#error "Invalid ST7789_ROTATION"
  ...
  /* CASET */
  {
    uint8_t c = 0x2A;
    ST7789_DC_LOW();
    st7789_tx_blocking(&c, 1);
    ST7789_DC_HIGH();
    st7789_tx_blocking(caset, 4);
  }
...
void ST7789_Init(void) {
  ...
  {
    uint8_t d[] = {0xD0, 0x08, 0x0E, 0x09, 0x09, 0x05, 0x31,
  ...
  {
    uint8_t d[] = {0xD0, 0x08, 0x0E, 0x09, 0x09, 0x15, 0x31,
                   0x33, 0x48, 0x17, 0x14, 0x15, 0x31, 0x34};
    ST7789_WriteData(d, sizeof(d));
  }
...
/* 填充矩形（尽量用整块 DMA） */
void ST7789_FillRect(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye,
                     uint16_t color) {
  if (xe < xs) {
    uint16_t t = xs;
    xs = xe;
 ...
 */
void ST7789_DrawBitLine16BPP(uint16_t xs, uint16_t y, const uint8_t *p,
                             uint16_t xsize) {
  if (y >= ST7789_HEIGHT || xs >= ST7789_WIDTH)
    return;
  if (xsize == 0)
    return;
  if (xs + xsize > ST7789_WIDTH)
    xsize = ST7789_WIDTH - xs;
...
void ST7789_DrawBitmap(uint16_t xs, uint16_t ys, uint16_t xsize, uint16_t ysize,
                       const uint8_t *p) {
  if (xs >= ST7789_WIDTH || ys >= ST7789_HEIGHT)
    return;
  if (xsize == 0 || ysize == 0)
...
void ST7789_ShowChar(uint16_t x, uint16_t y, uint8_t ch, FontDef font,
  ...
  /* 越界裁剪 */
  if (x + w - 1 >= ST7789_WIDTH || y + h - 1 >= ST7789_HEIGHT)
    return;
...
void ST7789_ShowString(uint16_t x, uint16_t y, const char *str, FontDef font,
                       uint16_t color, uint16_t bgcolor) {
  if (!str)
    return;
  /* 同样用局部变量保存宽高 */
...
void ST7789_TestFrameRate(void) {
  ...
  for (uint32_t f = 0; f < frame_count; f++) {
    uint16_t color =
        test_colors[f % (sizeof(test_colors) / sizeof(test_colors[0]))];
    ST7789_FillRect(0, 0, ST7789_WIDTH - 1, ST7789_HEIGHT - 1, color);
  }
  elapsed = HAL_GetTick() - tick_start;

  double fps =
      (elapsed > 0) ? ((double)frame_count * 1000.0) / (double)elapsed : 0.0;
...
static void ST7789_DrawVisibleText_SmoothRow(int16_t x, uint16_t y,
                                             const char *s, const FontDef *font,
                                             uint16_t color, uint16_t bgcolor) {
  if (!s || !font)
    return;
  const int16_t fw = (int16_t)font->width;
  const int16_t fh = (int16_t)font->height;
  if (y >= ST7789_HEIGHT || (int32_t)y + fh - 1 > (int32_t)ST7789_HEIGHT - 1)
    return;
  ...
  for (int16_t row = 0; row < fh; row++) {
    ...
    while (*p) {
      ...
      int16_t vis1 =
          (gx1 >= (int16_t)ST7789_WIDTH) ? ((int16_t)ST7789_WIDTH - 1) : gx1;
    ...
    ST7789_SetWindow(0, (uint16_t)(y + row), ST7789_WIDTH - 1,
                     (uint16_t)(y + row));
    ST7789_WriteCmd(0x2C);
...
/* 初始化（注意 font 传指针） */
void ST7789_MarqueeInit(ST7789_Marquee *m, const char *text,
                        const FontDef *font, uint16_t y, uint16_t color,
                        uint16_t bgcolor, int16_t speed_px, uint16_t gap_px,
...
void ST7789_MarqueeStep(ST7789_Marquee *m, uint32_t now_ms) {
  ...
  if (m->last_ms && (uint32_t)(now_ms - m->last_ms) < m->interval_ms)
    return;
  m->last_ms = now_ms;

  uint16_t band_y0 = m->y;
  uint16_t band_y1 = (uint16_t)(m->y + m->font->height - 1);
  if (band_y1 >= ST7789_HEIGHT)
    band_y1 = ST7789_HEIGHT - 1;

  /* 1) 擦除这一行带 */
  ST7789_FillRect(0, band_y0, ST7789_WIDTH - 1, band_y1, m->bgcolor);

  /* 2) 绘制当前帧 */
  ST7789_DrawVisibleText_SmoothRow(m->x, m->y, m->text, m->font, m->color,
                                   m->bgcolor);
...
/* 阻塞演示（方便快速验证） */
void ST7789_MarqueeRunBlocking(const char *text, const FontDef *font,
                               uint16_t y, uint16_t color, uint16_t bgcolor,
                               int16_t speed_px, uint16_t gap_px,
                               uint32_t interval_ms, uint32_t duration_ms) {
  ST7789_Marquee m;
  ST7789_MarqueeInit(&m, text, font, y, color, bgcolor, speed_px, gap_px,
                     interval_ms);
