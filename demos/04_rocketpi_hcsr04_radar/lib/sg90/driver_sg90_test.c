...
static void log_angle(float angle_deg) {
  ...
  int len = snprintf(uart_log_buffer[next_buffer],
                     sizeof(uart_log_buffer[next_buffer]),
                     "SG90 angle: %.2f\r\n", angle_deg);
  if (len <= 0) {
    return;
  }

  if (HAL_UART_Transmit_DMA(&huart2, (uint8_t *)uart_log_buffer[next_buffer],
                            (uint16_t)len) == HAL_OK) {
...
void SG90_Test_Scan(uint32_t step_delay_ms) {
  ...
  while (1) {
    ...
    for (uint32_t idx = 0U; idx <= max_index; ++idx) {
      ...
      uint32_t delay_ms =
          base_delay + ((idx % steps_per_degree) < remainder ? 1U : 0U);
      HAL_Delay(delay_ms);
    }
